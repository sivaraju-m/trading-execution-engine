...
class RiskManager:
    ...
    async def validate_signal(self, signal: Dict[str, Any]) -> bool:
        ...
        if self.daily_pnl < -self.daily_limits['max_daily_loss']:
            self._log_violation("daily_loss_limit_exceeded", {
    ...
    async def _validate_concentration_risk(self, symbol: str, position_value: float, action: str) -> bool:
        ...
        if concentration_pct > self.max_concentration_pct:
            self._log_violation("concentration_risk_exceeded", {
                'symbol': symbol,
    ...
    async def _validate_stop_loss(self, signal: Dict[str, Any]) -> bool:
        ...
        if stop_loss_pct > self.stop_loss_pct:
            self._log_violation("stop_loss_too_wide", {
                'signal': signal,
    ...
    async def update_position(self, symbol: str, action: str, quantity: int, price: float):
        ...
        logger.debug(f"Position updated: {symbol} - {position['quantity']} @ ₹{position['avg_price']:.2f}")
    ...
    async def update_pnl(self, pnl_change: float):
        ...
        if self.daily_pnl < -self.daily_limits['max_daily_loss']:
            self._log_violation("daily_loss_limit_breached", {
                'current_pnl': self.daily_pnl,
    ...
    async def generate_daily_summary(self) -> Dict[str, Any]:
        ...
        summary = {
            ...
            'risk_metrics': {
                ...
                'daily_loss_utilization_pct': abs(self.daily_pnl) / self.daily_limits['max_daily_loss'] * 100,
                'total_violations': len(self.risk_violations)
            }
        }
        
        logger.info(f"Risk summary: {len(self.risk_violations)} violations, "
    ...
    def _calculate_max_position_utilization(self) -> float:
        ...
        max_position_value = max([pos['value'] for pos in self.current_positions.values()], default=0)
        return (max_position_value / self.daily_limits['max_position_size']) * 100
    
    def _calculate_concentration_risk(self) -> float:
        """Calculate concentration risk percentage"""
        max_exposure = max([pos['value'] for pos in self.current_positions.values()], default=0)
