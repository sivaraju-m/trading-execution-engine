...
class PaperTrader:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        logger.info(f"Paper trader initialized with capital: ₹{self.initial_capital:,.2f}")
    ...
    async def execute_signal(self, signal: Dict[str, Any]) -> Dict[str, Any]:
        ...
        logger.info(f"Paper trade executed: {action} {quantity} {symbol} @ ₹{execution_price:.2f} "
                   f"(P&L: ₹{pnl:.2f})")
    ...
    def _update_portfolio(self, trade: Dict[str, Any]):
        ...
        if action == 'BUY':
            ...
            if position['quantity'] >= quantity:
                ...
                logger.warning(f"Attempting to sell {quantity} {symbol}, but only have {position['quantity']}")
    ...
    async def close_daily_session(self) -> Dict[str, Any]:
        ...
        portfolio_value = sum(pos['quantity'] * pos['avg_price'] for pos in self.portfolio.values())
        summary['portfolio_value'] = portfolio_value
        summary['total_value'] = self.available_capital + portfolio_value
        summary['return_pct'] = (summary['total_value'] - self.initial_capital) / self.initial_capital * 100
        
        logger.info(f"Paper trading session closed: {summary['total_trades']} trades, "
