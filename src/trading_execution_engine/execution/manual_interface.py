...
class ManualExecutionInterface:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.cooling_period = config.get('cooling_period_minutes', 5) * 60  # Convert to seconds
    ...
    async def present_signal(self, signal: Dict[str, Any]) -> Dict[str, Any]:
        ...
        result = {
            ...
            'execution_time': datetime.now().isoformat() if user_decision['executed'] else None,
        ...
        if user_decision['executed']:
            ...
            # Remove from pending
            self.pending_signals = [s for s in self.pending_signals if s['signal_id'] != signal_id]
    ...
    async def _simulate_user_decision(self, signal: Dict[str, Any]) -> Dict[str, Any]:
        ...
        will_execute = (
            strength > 0.7 and  # High confidence signals only
            estimated_value <= self.max_order_value and  # Within order limits
            signal.get('action', '').upper() in ['BUY', 'SELL']  # Valid actions
        )
        
        if will_execute:
            # Simulate manual execution with slight variations
            price_adjustment = 0.99 if signal.get('action', '').upper() == 'BUY' else 1.01
    ...
    async def export_daily_summary(self) -> Dict[str, Any]:
        ...
        summary = {
            ...
            'total_signals_presented': len(self.pending_signals) + len(self.executed_trades),
            'signals_executed': len(self.executed_trades),
            'execution_rate': len(self.executed_trades) / max(1, len(self.pending_signals) + len(self.executed_trades)),
            'total_trade_value': sum(trade.get('estimated_value', 0) for trade in self.executed_trades),
            'executed_trades': self.executed_trades,
            'pending_signals': self.pending_signals
        }
        
        logger.info(f"Manual trading summary: {summary['signals_executed']} trades executed "
    ...
    async def mark_signal_executed(self, signal_id: str, execution_details: Dict[str, Any]) -> bool:
