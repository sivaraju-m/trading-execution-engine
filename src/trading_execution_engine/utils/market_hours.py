...
class MarketHoursValidator:
    ...
    def get_trading_session_info(self, check_date: date = None) -> dict:
        ...
        if session_info['is_trading_day']:
            session_info.update({
                'market_open': datetime.combine(check_date, self.market_open).isoformat(),
                'market_close': datetime.combine(check_date, self.market_close).isoformat(),
                'pre_market_start': datetime.combine(check_date, self.pre_market_start).isoformat(),
                'post_market_end': datetime.combine(check_date, self.post_market_end).isoformat()
            })
