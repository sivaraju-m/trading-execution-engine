...
from src.trading_execution_engine.execution.manual_interface import (
    ManualExecutionInterface,
)
from src.trading_execution_engine.execution.paper_trader import PaperTrader
from src.trading_execution_engine.monitoring.performance_tracker import (
...
class DailyTradingScheduler:
    ...
    def __init__(self, config_path: Optional[str] = None):
        ...
        self.manual_interface = ManualExecutionInterface(
            self.config.get("manual_trading", {})
        )
    ...
    async def run_daily_trading_cycle(self) -> Dict[str, Any]:
        ...
        logger.info(
            f"Daily trading cycle completed: {results['total_signals']} signals, "
    ...
    async def _run_trading_cycle(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for signal in signals:
                ...
                # Track performance
                await self.performance_tracker.track_signal(
                    signal,
                    {
                        "paper_result": (
                            paper_result if "paper_result" in locals() else None
                        ),
                        "manual_result": (
                            manual_result if "manual_result" in locals() else None
                        ),
                    },
                )
...
def main():
    ...
    parser.add_argument(
        "--mode",
        choices=["paper", "manual", "both"],
        default="both",
        help="Trading mode",
    )
    parser.add_argument(
        "--dry-run", action="store_true", help="Run without executing any trades"
    )
